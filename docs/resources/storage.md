...
Do not point this resource at the root disk of an instance, it is managed by `storage_size_gb` and
`storage_policy_id` on `fptcloud_instance`. On OpenStack the root disk is listed with
`type = LOCAL`, so it is easy to pick it up by mistake.
