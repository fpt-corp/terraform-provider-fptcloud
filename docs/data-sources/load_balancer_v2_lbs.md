...
-> Note: `loadbalancers.*.tags` is an internal marker identifying the object as LBv2 for Portal, not FPT Cloud's tagging service. For the real tags (from Portal's "Manage Tags"), use `loadbalancers.*.resource_tags` instead. See the singular [`fptcloud_load_balancer_v2_lb`](load_balancer_v2_lb.md#tags-vs-resource_tags) data source docs for details.
