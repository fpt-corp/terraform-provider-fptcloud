...
Note the unusually long Required list below. This data source looks a cluster
up by `cluster_name` within `vpc_id`, but it shares its model with the
resource, so several attributes the lookup does not actually use are still
marked required. Give them the values the cluster was created with.
...
- `auto_scale` (Boolean) Always false — a bare-metal pool has a fixed server count, not a range
